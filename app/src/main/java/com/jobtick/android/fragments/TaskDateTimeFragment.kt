            ...
            override fun callDraftTaskDateTime(taskModel: TaskModel?) {
                if (taskModel!!.dueDate != null) {
                    ...
                    if (!TextUtils.isEmpty(txtDate!!.trim { it <= ' ' }) && !checkDateTodayOrOnwords()) {
                        taskModel.dueDate = Tools.getDayMonthDateTimeFormat(txtDate!!.trim { it <= ' ' })
                    }
...
//        }
    ...
    private fun showSpinner() {
        ...
        spinnerArrow.setImageDrawable(ContextCompat.getDrawable(requireActivity(), R.drawable.ic_up_arrow_g))
        isSpinnerOpen = true
    ...
    private fun hideSpinner() {
        isSpinnerOpen = false
        spinnerArrow.setImageDrawable(ContextCompat.getDrawable(requireActivity(), R.drawable.ic_d_arrow_g))
        edtTimeSpinner.setTextColor(ContextCompat.getColor(requireActivity(), R.color.N9001))
        spinnerItems.visibility = View.GONE
    }

    companion object {
        fun newInstance(due_date: String?, due_time: DueTimeModel?, operationsListener: OperationsListener?): TaskDateTimeFragment {
            val args = Bundle()
